...
def loss(X, Y):
    ...
    SigCE = tf.nn.sigmoid_cross_entropy_with_logits(labels=Y, logits=Yhat)  # Sigmoid Cross Entropy
    loss = SigCE  # Total Loss
    return loss
...
def evaluate(Xtest, Ytest):
    ...
    accuracy = tf.reduce_mean(tf.cast(tf.equal(Y_predicted, tf.cast(Ytest, tf.float32)), tf.float32))
    return accuracy
...
with tf.Session() as sess:
    ...
    print('Binary Prediction = ', Ytest_predicted.eval(), ' --- Prediction Probability = ', Ytest_inference.eval())
